...
def export_model(model, path=None, input_shape=(1, 3, 64, 64), cuda=False):
    """
...
class SequentialDLIBDataset(Dataset):
    """
    Like `DLIBDataset`, but indexes the dataset sequentially by the latent factors

    Note that if the dataset has observation factors, those are still randomly
    sampled.
    """
    ...
    def __getitem__(self, item):
        assert item < self.iterator_len
        factors = np.unravel_index(item, self.dataset.factors_num_values)
        factors = np.array(factors)[np.newaxis, :]
        output = self.dataset.sample_observations_from_factors(factors, self.random_state)[0]
        # Convert output to CHW from HWC
        return torch.from_numpy(np.moveaxis(output, 2, 0))


class DLIBDatasetWithClasses(Dataset):
    """
    Like `DLIBDataset`, but returns the factors as classes
    """
    def __init__(self, name, seed=0):
        self.name = name
        self.random_state = np.random.RandomState(seed)
    ...
    def __getitem__(self, item):
        assert item < self.iterator_len
        factors = np.array(np.unravel_index(item, self.dataset.factors_num_values))
        output = self.dataset.sample_observations_from_factors(factors[np.newaxis, :],
                                                               self.random_state)[0]
        # Convert output to CHW from HWC
        return torch.from_numpy(np.moveaxis(output, 2, 0)).float(), factors


def get_dataset(name=None, seed=0, iterator_len=50000, sequential=False):
    """
    Makes a dataset.
    ...
    iterator_len : int
        Length of the dataset. This defines the length of one training epoch.
    sequential : bool
        If true, use a sequential dataset instead of the random indexing one.
        In this case, iterator_len has no effect.
    ...
    if sequential:
        return SequentialDLIBDataset(name, seed=seed)
    else:
...
def get_loader(name=None, batch_size=32, seed=0, iterator_len=50000, num_workers=0,
               sequential=False, shuffle=True, **dataloader_kwargs):
    """
    ...
    name = get_dataset_name() if name is None else name
    if sequential:
        dlib_dataset = SequentialDLIBDataset(name, seed=seed)
    else:
        dlib_dataset = DLIBDataset(name, seed=seed, iterator_len=iterator_len)
    loader = DataLoader(dlib_dataset, batch_size=batch_size, shuffle=shuffle,
